...
class SplitData:
    ...
    def prepare_autoencoder_data(self) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, pd.Series]:
        ...
        if self.print_codes_:
            logging.info(f'Train codes: {df_train.Unique_Code.astype(str).unique()}\n')
            logging.info(f'Val codes: {df_val.Unique_Code.astype(str).unique()}\n')
            logging.info(f'Test codes: {df_test.Unique_Code.astype(str).unique()}\n')
        ...
        # logging.info(f'OBS: Returning validation unique codes...\n')
