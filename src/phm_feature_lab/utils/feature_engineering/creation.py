...
class FeatureCreationUtils:
    ...
    @staticmethod
    def _rms_freq(freqs: np.ndarray, amplitudes: np.ndarray) -> float:
        
        return np.sqrt(np.mean(amplitudes ** 2))
    
    @staticmethod
    def _mean_freq(freqs: np.ndarray, amplitudes: np.ndarray) -> float:
        """Calculate the mean frequency.
        """
        return np.sum(freqs * amplitudes) / np.sum(amplitudes)

    @staticmethod
    def _std_freq(freqs: np.ndarray, amplitudes: np.ndarray) -> float:
        """Calculate the standard deviation of frequency.
        """
        center_freq = FeatureCreationUtils._mean_freq(freqs, amplitudes)
        return np.sqrt(np.sum((freqs - center_freq) ** 2 * amplitudes) / np.sum(amplitudes))

    @staticmethod
    def _skew_freq(freqs: np.ndarray, amplitudes: np.ndarray) -> float:
        """Calculate the spectral skewness.
        """
        center_freq = FeatureCreationUtils._mean_freq(freqs, amplitudes)
        std_freq = FeatureCreationUtils._std_freq(freqs, amplitudes)
        return np.sum(((freqs - center_freq) / std_freq) ** 3 * amplitudes) / np.sum(amplitudes)

    @staticmethod
    def _kurt_freq(freqs: np.ndarray, amplitudes: np.ndarray) -> float:
        """Calculate the spectral kurtosis.
        """
        center_freq = FeatureCreationUtils._mean_freq(freqs, amplitudes)
        std_freq = FeatureCreationUtils._std_freq(freqs, amplitudes)
        return np.sum(((freqs - center_freq) / std_freq) ** 4 * amplitudes) / np.sum(amplitudes)
    
    @staticmethod
    def _max_freq(freqs: np.ndarray, amplitudes: np.ndarray) -> float:
        """Calculate the frequency corresponding to the maximum amplitude."""
        if np.sum(amplitudes) == 0:
            return 0.0  # Return 0 if amplitudes sum to zero (no energy)
        peak_index = np.argmax(amplitudes)
        return freqs[peak_index]
    
    @staticmethod
    def _get_freq_metrics(window: pd.Series, fs: int) -> pd.Series:
        """Calculate all frequency metrics for a single window."""
        freqs, amplitudes = perform_fft(window.values, fs)
        return {
            'rms_freq': FeatureCreationUtils._rms_freq(freqs, amplitudes),  # Root Mean Square Frequency
            'men_freq': FeatureCreationUtils._mean_freq(freqs, amplitudes),  # Mean Frequency
            'std_freq': FeatureCreationUtils._std_freq(freqs, amplitudes),  # Standard Deviation of Frequency
            'skew_freq': FeatureCreationUtils._skew_freq(freqs, amplitudes),   # Spectral Skewness
            'kurt_freq': FeatureCreationUtils._kurt_freq(freqs, amplitudes),   # Spectral Kurtosis
            'max_freq': FeatureCreationUtils._max_freq(freqs, amplitudes)     # Peak Frequency
        }
    ...
    @staticmethod
    def statistical_frequency_features(
        data: pd.Series, 
        window_size: int, 
        step_size: int, 
        min_periods: int,
        ...
        def calculate_metrics(window: pd.Series) -> dict:
            """Calculate all frequency metrics for a single window."""
            freqs, amplitudes = perform_fft(window.values, fs)
            return {
                'rms_freq': FeatureCreationUtils._rms_freq(freqs, amplitudes),  # Root Mean Square Frequency
                'mean_freq': FeatureCreationUtils._mean_freq(freqs, amplitudes),  # Mean Frequency
                'std_freq': FeatureCreationUtils._std_freq(freqs, amplitudes),  # Standard Deviation of Frequency
                'skew_freq': FeatureCreationUtils._skew_freq(freqs, amplitudes),   # Spectral Skewness
                'kurt_freq': FeatureCreationUtils._kurt_freq(freqs, amplitudes),   # Spectral Kurtosis
                'max_freq': FeatureCreationUtils._max_freq(freqs, amplitudes)     # Peak Frequency
        ...
        funcs = {
            'rms_freq': lambda x: calculate_metrics(x)['rms_freq'],
            'mean_freq': lambda x: calculate_metrics(x)['mean_freq'],
            'std_freq': lambda x: calculate_metrics(x)['std_freq'],
            'skew_freq': lambda x: calculate_metrics(x)['skew_freq'],
