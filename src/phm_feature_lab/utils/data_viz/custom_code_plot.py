...
class CustomCodePlot:
    ...
    @staticmethod
    def __normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def __get_year_month_labels(self) -> List[str]:
        ...
        if self.__group_set == "train":
            return CustomCodePlot.TRAIN_MONTHS
        elif self.__group_set == "test":
            return CustomCodePlot.TEST_MONTHS
        
        return CustomCodePlot.TRAIN_MONTHS + CustomCodePlot.TEST_MONTHS
    ...
    def __filter_data(self, machine: str, operation: str, axis: Optional[Union[str, List[str]]]) -> pd.DataFrame:
        ...
        self.__ensure_column_exists(self.__df, "axis")

        # Se 'axis' for uma string, converta para lista
        if isinstance(axis, str):
            axis = [axis]
        
        filtered_df = self.__df[self.__df["axis"].isin(axis)]  # Filtra para múltiplos eixos
    ...
    @staticmethod
    def __calculate_y_axis_limits(df: pd.DataFrame, var: str, y_min: Optional[float], y_max: Optional[float]) -> tuple:
        """
    ...
    def __plot_axis(
        ...
        df_filtered = self.__filter_data(machine, operation, ax_name)
        df_metadata = self.__prepare_data(df_filtered, machine, operation, var)
        y_min_local, y_max_local = self.__calculate_y_axis_limits(
            df_metadata, var, y_min, y_max
        )

        for j in range(len(df_metadata) - 1):
            ax.plot(
                df_metadata["unique_code"].iloc[j : j + 2],
                df_metadata[var].iloc[j : j + 2],
                linestyle="-",
        ...
        self.__plot_last_point(ax, df_metadata, var, anomaly_column)
        ax.set_ylim(y_min_local, y_max_local)
        ax.set_ylabel(y_axis_label if y_axis_label else f"{var}")
        ax.set_title(title if title else f"{machine}-{operation} ({ax_name})")
        ax.grid(True)

    def __prepare_data(self, df, machine, operation, var):
    ...
    def __plot_last_point(self, ax, df_metadata, var, anomaly_column):
        ...
        is_anomaly_last = (
            anomaly_column and df_metadata[anomaly_column].iloc[-1] == "Anomaly"
        )
        ax.scatter(
