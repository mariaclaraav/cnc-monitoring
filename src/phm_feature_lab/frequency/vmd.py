"""
Created on Sunday May 11 2025

@author: Maria Clara Assunção Viana
"""
import numpy as np

def  VMD(f, alpha, tau, K, DC, init, tol, icfs, fs_real):
    """
    ...
    if icfs is not None:
        omega_plus[0, :] = np.array(icfs) / fs_real  # normalizando pelas unidades do domínio espectral
    elif init == 1:
        for i in range(K):
