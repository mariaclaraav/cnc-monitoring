...
import pandas as pd
from phm_feature_lab.features.frequency_analyzer import FrequencyAnalyzer
...
class CustomProcessor:
    ...
    def log_memory_usage(self, message: str):
        ...
        logger.info(f"{message} | Memory usage: {mem_usage:.2f} MB")
    ...
    def filter_dataframe(
        ...
        try:
            ...
            if not required_columns.issubset(df.columns):
                logger.error(f"Missing columns in DataFrame. Found: {df.columns}")
                raise ValueError(f"DataFrame must contain {required_columns}")
            
            # Filter the DataFrame
            df_filtered = df[df['Process'] == operation].copy()
            if df_filtered.empty:
                logger.error(f"No rows found for operation: {operation}")
            ...
            logger.debug(f"Filtered columns: {df_filtered.columns}")
            ...
            if df_filtered['Month'].isnull().any():
                logger.error("Mapping Month to numeric values resulted in NaN values.")
            ...
            logger.info(f"Data filtered successfully. Shape: {df_filtered.shape}")
            del df
            gc.collect()
            return df_filtered
            ...
            logger.error(f"Operation column unique values: {df['Process'].unique()}")
            raise
    ...
    def process_features(
        ...
        frequency_by_axis = {}           
        processed_data = {}
        processing_times = {}  # To store processing times for each feature type

        # Get the number of unique codes
        num_unique_codes = df_filtered['Unique_Code'].nunique()
        
        for feature_type in feature_types:
            logger.info(f"Calculating {feature_type} data...")
            ...
            # Calculate average processing time per Unique_Code
            avg_time_per_code = elapsed_time / num_unique_codes
            processing_times[feature_type] = avg_time_per_code

            # Log the average processing time
            logger.info(f"Average processing time for {feature_type} per Unique_Code: {avg_time_per_code:.4f} seconds")
